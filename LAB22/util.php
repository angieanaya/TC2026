...
//Crea un select con los datos de una consulta
  ...
  function crear_select($id, $columna_descripcion, $tabla, $required, $selected=0) {
    ...
    $resultado = '<div class="input-field"><select name="'.$tabla.'" id="'.$tabla.'"><option value="" selected>Selecciona una opción</option>';
  ...
  function insertarLibro($Titulo, $idGenero, $idAutor){
    $conexion_bd = conectDB();

    //Preparar consulta

    $registrar = 'CALL creaLibro(?,?,?)';
    ...
    //Unir los parámetros
    if (!$statement->bind_param("sii", $Titulo, $idGenero, $idAutor)){
